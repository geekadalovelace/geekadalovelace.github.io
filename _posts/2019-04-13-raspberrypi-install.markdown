...
BerryBoot 가 복사된 메모리 카드를 라즈베리 파이에 꼽고 부팅을 합니다. 처음 부팅하면 화면, 지역, 네트워크 등의 설정 창이 뜨는데 큰 어려움 없이 설정을 할 수 있습니다.

##### Ubuntu MATE, RetroPie 설치

준비한 USB 메모리에 다운받은 Ubuntu MATE 를 압축해져하여 나온 img 파일과 RetroPie 의 img 를 복사합니다. USB 메모리의 파일 시스템은 FAT32 여야 합니다.
...
그 후 다시 리눅스 Package 업데이트를 마져 수행합니다.
...
이제는 레트로 파이를 설정할 차례입니다. 라즈베리파이를 재부팅 시킨후 레트로  파이로 부팅을 합니다. 엑박 패드를 사용하기 위해서 USB 에 연결하고 부팅을 하였습니다. 조이스틱 모양의 아름다운 아이콘이 나오면서 부팅이되면 패트의 키 설정을 하게 됩니다. 설정을 완료하고 재부팅을 하면 에뮬레이터를 선택하는 화면이 나타납니다.

루카스 어드벤처 게임을 돌릴 수 있는 스컴vm 도 있네요. 인터넷, 블루투스 스피커 설정을 위해서 조이스틱 모양의 RetroPie 를 실행시킵니다.

저는 무선 인터넷을 설정하기 위해서 CONFIGURE WIFI 를 선택하였습니다.

헛! 그런데 이미 설정이 잘 잡혀있네요. 삼바 설정를 위해 F4 를 누르고 쉘로 진입합니다.
